...
def boundary_point(x, on_boundary):
    return on_boundary and np.isclose(x[0], 0.5) and np.isclose(x[1], 0.5)
...
bc_point = dde.icbc.DirichletBC(geomtime, lambda x: 1, boundary_point)
...
#     lambda x, y: torch.ones_like(y) if torch.allclose(x[0], torch.tensor(0.5)) and torch.allclose(x[1], torch.tensor(0.5)) else y
# )
...
def anim_update(t_id):
    ...
    plt.savefig(work_path + '//' + 'Try' + str(t_id) + '.png')
...
anim.save(work_path + "//" + "Try" + str(Nt + 1) + ".gif", writer="pillow", dpi=300)
